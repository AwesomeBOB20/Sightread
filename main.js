...
// [START OF SNIPPET 2 REPLACEMENT - normalizeSixteenthGridBeat]
...
    
    // Helper to check for a single 16th or 16th rest with no dots
    const isPlain16 = (e) => 
        e && (e.dur === "16" || e.dur === "16r") && 
        Math.abs((e.beats ?? 0) - 0.25) < eps && Number(e.dots || 0) === 0;

    // We only perform complex analysis if we have exactly four 16th subdivisions
    const isFourSixteenths = beat.length === 4 && beat.every(isPlain16);

    // --- Handling complex edge cases (3-element patterns) that might result from other functions ---
    if (!isFourSixteenths) {
        if (beat.length === 3) {
            const is16r = (e) => e && e.kind === "rest" && e.dur === "16" && Math.abs((e.beats ?? 0) - 0.25) < eps && Number(e.dots || 0) === 0;
            const isPlain8 = (e) => e && (e.kind === "note" || e.kind === "rest") && e.dur === "8" && Math.abs((e.beats ?? 0) - 0.5) < eps && Number(e.dots || 0) === 0;

            if (is16r(beat[0]) && is16r(beat[1]) && isPlain8(beat[2])) {
                return [R("8", 0.5), beat[2].kind === "note" ? N("8", 0.5) : R("8", 0.5)];
            }
            if (isPlain8(beat[0]) && is16r(beat[1]) && is16r(beat[2])) {
                return [beat[0].kind === "note" ? N("8", 0.5) : R("8", 0.5), R("8", 0.5)];
            }
        }
        // If it's not exactly 4x16ths and not one of the 3-element exceptions, return as is.
        return beat;
    }
    // --- End of Edge Cases ---

    // Now we know we have exactly four 16th-subdivisions (N/R)
    const pat = beat.map((e) => (e.kind === "note" ? "n" : "r")).join("");

    if (pat === "rnrr") return [R("16", 0.25), N("8", 0.75, 1)]; // 16R + Dotted 8N
    if (pat === "rrrn") return [R("8", 0.75, 1), N("16", 0.25)]; // Dotted 8R + 16N
    if (pat === "nrrn") return [N("8", 0.75, 1), N("16", 0.25)]; // Dotted 8N + 16N
    if (pat === "nnrr") return [N("16", 0.25), N("16", 0.25), R("8", 0.5)]; // Two 16N + 8R
    if (pat === "nrrr") return [N("q", 1)]; // Quarter Note

    // YOUR EXPLICITLY REQUESTED PATTERN: R R N N -> 8th Rest + 16th Note + 16th Note
    if (pat === "rrnn") return [R("8", 0.5), N("16", 0.25), N("16", 0.25)]; 

    return beat;
}
// [END OF SNIPPET 2 REPLACEMENT - normalizeSixteenthGridBeat]
  ...
  function normalizeEighthRestEighth(beat) {
    ...
    // [START OF SNIPPET 1 REPLACEMENT - normalizeEighthRestEighth]
...
    // The explicit simplification rule converting [8r, 16n, 16n] to [8r, 8n] 
    // is removed here to allow the desired pattern to survive when generated.
...
    return out;
  }
// [END OF SNIPPET 1 REPLACEMENT - normalizeEighthRestEighth]
...
function applySticking(exercise, strategy) {
    ...
    // If global toggle is off, stop here
    if (!isStickingVisible) return;

    // Use Global Lead Hand
    ...
    // NATURAL STICKING (Grid-Based with Triplet Flip)
    if (strategy === "natural") {
      let currentBeatLead = globalLead; // Tracks the lead hand for the *current beat*

      exercise.forEach(m => {
        m.beats.forEach(beat => {
          const isTriplet = !!beat._tuplet;
          
          // Determine if we flip for the NEXT beat
          // Rule: Odd groupings (Triplets) flip the lead. Even groupings (Straight 16ths) keep it.
          const shouldFlip = isTriplet; 

          // Apply sticking to notes within this beat based on the *current* beat lead
          // We use a GRID so that rests don't mess up the sticking order (e.g. "Rest-la-li")
          let pos = 0;
          beat.forEach(n => {
            if (n.kind === "note") {
              let stick = currentBeatLead;
              const p = pos % 1; // Normalize position within the beat (0.0 to 0.99)
              const eps = 0.05;  // Tolerance for float math

              if (isTriplet) {
                // Triplet Grid: 1(Lead) la(Other) li(Lead)
                // 0.00 = Lead
                // 0.33 = Other
                // 0.66 = Lead
                if (Math.abs(p - 1/3) < eps) stick = other(currentBeatLead); // 'la'
                else stick = currentBeatLead; // '1' or 'li'
              } else {
                // Straight Grid: 1(Lead) e(Other) &(Lead) a(Other)
                // 0.00 = Lead
                // 0.25 = Other
                // 0.50 = Lead
                // 0.75 = Other
                const isSixteenthOff = (Math.abs(p - 0.25) < eps || Math.abs(p - 0.75) < eps);
                if (isSixteenthOff) stick = other(currentBeatLead);
          ...
          // Advance state for the next beat
          if (shouldFlip) {
            currentBeatLead = other(currentBeatLead);
          }
        });
      });
    }
...
function makeStaveNote(flow, elem, beatIdx, posInBeat, isTriplet) {
    ...
    // 1. STICKING (Moved to BOTTOM)
    // We add this first so it sits closest to the note head
    ...
    // 2. COUNTS (Moved to BOTTOM, stacking below sticking)
    // FIX: Only show counts if user wants them AND it is NOT a rest
    if (currentShowCounts && !isRest) {
       ...
       if (txt) {
           const countAnn = new flow.Annotation(txt)
            // UPDATED: Same style as Sticking (Bold Arial)
            .setFont("Arial", 11, "bold") 
            .setVerticalJustification(flow.Annotation.VerticalJustify.BOTTOM);
  ...
  function buildMeasure(flow, measureModel) {
    ...
    for (let beatIdx = 0; beatIdx < 4; beatIdx++) {
      ...
      const isTripletBeat = !!beat._tuplet; // Check before loop

      // NEW: Pass beat info to makeStaveNote so Counts can be calculated
      let runningPos = 0;
      const vfNotes = beat.map((e) => {
          const n = makeStaveNote(flow, e, beatIdx, runningPos, isTripletBeat);
          // Store absolute beat position for Playhead logic
  ...
  function render(exercise) {
    ...
    // Pass true to packMeasure if it's the very first one (for Clef padding)
    const packs = exercise.map((mm, i) => packMeasure(flow, mm, i === 0));

    // 1. Calculate Widths
    // Use getBoundingClientRect for sub-pixel accuracy (fixes some mobile overflow issues)
    ...
    if (availW < mobileBreakpoint) {
        // Mobile: Zoom out further (0.65) to ensure complex measures 
        // fit on narrow screens without pushing off-canvas.
        scale = 0.65; 
    }

    let wrapW = availW; 

    // Usable width for the staves IN VIRTUAL SPACE (unscaled)
    let usableW = (wrapW / scale) - (marginX * 2);

    // 2. Line Wrapping Strategy
    const totalMinW = packs.reduce((sum, p) => sum + p.minW, 0);
    const avgMinW = totalMinW / totalMeasures;

    // Constraint A: Density
    const densityLimit = Math.floor(usableW / (avgMinW * 1.3));

    // Constraint B: Fixed Pixel Minimum (Don't allow measures smaller than 250px)
    const pixelLimit = Math.floor(usableW / 250);

    // Constraint C: Hard Cap
    // CHANGED: Increased from 3 to 4. 
    // This allows measures to be narrower (25% width) on desktop, fixing the "too wide" look.
    ...
    // Width matches the container exactly so it never overflows
    const physW = Math.floor(wrapW);
...
scoreEl.style.height = physH + "px";
    ...
    for (let line = 0; line < linesCount; line++) {
      ...
      // Calculate how many measures on THIS line
      const remainingMeasures = totalMeasures - mIdx;
      const remainingLines = linesCount - line;
      const countOnLine = Math.ceil(remainingMeasures / remainingLines);
      
      // Exact width for each measure on this line to FILL the space
      ...
      for (let i = 0; i < countOnLine; i++) {
        ...
        // JUSTIFICATION: Use the full stave width
        const formatter = new flow.Formatter();
...
// Treat as resume if paused OR if we have scrubbed forward (accumulatedBeat > 0)
  ...
  function regenerate() {
    try {
      ...
      // FIX: Use the global variable, not the old element
      applySticking(currentExercise, currentStickingStrategy);
