...
function pickBeatPattern({ restPct, allowQuarters, allow8ths, allow16ths, allowTriplets, allowQuarterTriplets, allowQuintuplets, allowSextuplets }) {
    ...
    if (choice === "5let") {
        ...
        if (restPct === 0) {
            const mk5 = (d) => ({ kind: "note", dur: d, beats: d==="q"?0.8:0.4, _localTuplet: true });
            const vars = [
                // Standard
                ["8","8","8","8","8"],
                ...
                ["q","q","8"],     // 10101
                ["q","8","q"],     // 10110
                ["8","q","q"]      // 1101 (Assumed from "1101" / 0111)
            ];
...
let currentBeatLead = globalLead;
      ...
      exercise.forEach(m => {
        m.beats.forEach(beat => {
          // FIX: Ignore empty spacer beats (used for 2-beat rhythms like Quarter Triplets).
          // This prevents the code from "flipping" the hand logic during the empty space.
          ...
          // Only treat as 'Standard Triplet' if it's NOT a simplified 6-let
          const isStandardTriplet = (numNotes === 3) && !beat._sixLetGrid; 
          
          // HIERARCHY CHECK:
          const global6 = allowSextupletsEl && allowSextupletsEl.checked;
          const global3 = allowTripletsEl && allowTripletsEl.checked;
          ...
          if (is5Let || has6 || isTrip || isQT) {
              ...
              // UPDATED PRIORITY: Check for Triplets BEFORE Global 6-let check
              // This ensures Triplets stay alternating (R L R) even if 6-lets are on.
              else if (isTrip || isQT) {
                  gridUnit = 1/3; 
                  
                  if (isQT) {
                      // Quarter Triplets -> R R R (Non-Alternating)
                      shouldFlip = false;
                  } else {
                      // Standard Triplets -> R L R (Flip)
                      shouldFlip = true;
                  }
              } 
              else if (global6 || has6) {
                  // PRIORITY 3: 6-let Grid (R L R L R L)
                  gridUnit = 1/6; 
                  shouldFlip = false; 
              } 
              // ...
              else if (isQT) {
                  // PRIORITY 3: Only Quarter Triplets Allowed
                  // UPDATED: User wants "RRR to R" (Non-Alternating).
                  // We use the standard 1/3 Triplet grid (Positions 0, 2, 4).
                  // Even number of slots consumed -> No Flip.
                  gridUnit = 1/3; 
                  shouldFlip = false; 
              }
          } 
          // --- BINARY LOGIC (Quarters, 8ths, 16ths) ---
              ...
              if (useStrictGrid) {
                  ...
                  // Quarter Note Only Mode -> Non-Alternating (R R R R)
                  gridUnit = 1.0; 
                  shouldFlip = false; // <--- CHANGED: No longer flips
              }
          }
  ...
  function generateExercise({ measures, timeSignatures, restPct, allowQuarters, allow8ths, allow16ths, allowTriplets, allowQuarterTriplets, allowQuintuplets, allowSextuplets }) {    
    ...
    for (let m = 0; m < measures; m++) {
      ...
      const beatsPerMeasure = parseInt(ts.split("/")[0], 10);
      ...
      while (!hasNote && retries < 10) {
          ...
          while (b < beatsPerMeasure) {
            ...
            let beat = pickBeatPattern({ 
                restPct, 
                allowQuarters, // <--- FIXED: Now correctly passed
                allow8ths, 
                allow16ths, 
                allowTriplets, 
      ...
      if (!hasNote) {
           beats = [];
           for(let k=0; k<beatsPerMeasure; k++) beats.push([{ kind: "note", dur: "q", beats: 1 }]);
      }
...
function makeStaveNote(flow, elem, beatIdx, posInBeat, tupletType) {
  ...
  function packMeasure(flow, measureModel, isFirstMeasure = false) {
    ...
    const numBeats = parseInt(pack.timeSig.split("/")[0], 10);

    const voice = new flow.Voice({ num_beats: numBeats, beat_value: 4 });
    voice.setStrict(false);
    voice.addTickables(pack.notes);

    // CHANGED: Aggressively increased weights for 6-let spacing
    // We increased PER_NOTE (40->50) and nearly doubled PER_TUPLET (80->150).
    // This makes measures with tuplets "cost" much more, forcing the layout engine
    // to break lines sooner (e.g. only 2 measures per line instead of 3 or 4),
    // giving them ample space to expand without brackets overlapping.
    const BASE = 140 * SHEET_DENSITY;       
    const PER_NOTE = 50 * SHEET_DENSITY;   // Increased to 50
    const PER_TUPLET = 150 * SHEET_DENSITY; // Increased massively to 150
  ...
  function render(exercise) {
    ...
    // Get the container's visual width (guard if #scoreWrap is missing)
...
const padR = styles ? (parseFloat(styles.paddingRight) || 0) : 0;
    ...
    // On Mobile, we shrink it (0.65) to prevent edge spilling.
    let scale = 0.85; 
    if (availW < 1000) {
        scale = 0.8; 
    }
    if (availW < 700) {
        scale = 0.65; // UPDATED: Significantly smaller for mobile
    }
    ...
    for (let line = 0; line < linesCount; line++) {
      ...
      for (let i = 0; i < countOnLine; i++) {
        ...
        // Draw Time Sig on First Measure OR if it changed
        ...
        const lenBeats = parseInt((pack.timeSig || "4/4").split("/")[0], 10);
...
layoutMeasures[mIdx] = { 
  ...
  function flattenEvents(exercise) {
    ...
    const actualTotalBeats = exercise.reduce((acc, m) => {
        ...
        return acc + parseInt(ts.split("/")[0], 10);
    }, 0);

    // 2. Create array with exact size needed
    const events = Array.from({ length: actualTotalBeats }, () => []);
    ...
    exercise.forEach(measure => {
        measure.beats.forEach(beatGroup => {
            ...
            beatGroup.forEach(note => {
               if (note.kind === 'note') {
                   // UNIFIED PITCH: All notes are now 600Hz
                   // We removed the L/R check so everything sounds consistent
                   const freq = 600; 
                   
                   if (events[globalBeatIndex]) {
                       events[globalBeatIndex].push({ offset: posInBeat, freq: freq });
                   }
               }
  ...
  function scheduleBeats() {
    ...
    // NEW: Calculate Count-off Start (Negative Downbeat)
    const firstLen = (currentExercise && currentExercise[0])
        ? parseInt((currentExercise[0].timeSig || "4/4").split("/")[0], 10)
        : 4;
    const countInStart = -firstLen;
    ...
    while (nextNoteTime < audioCtx.currentTime + LOOKAHEAD) {
      ...
      if (nextBeatIndex < totalBeatsScheduled) {
           ...
           // UPDATED: isDownbeat is true if it's a measure start OR the very first count-off click
           const isDownbeat = starts.includes(nextBeatIndex) || (nextBeatIndex === countInStart);
           ...
           // RHYTHM NOTES:
           // Always use the frequency stored in flattenEvents (now fixed at 600Hz)
           if (nextBeatIndex >= 0 && nextBeatIndex < eventsByBeat.length) {
             ...
             for (const n of beatNotes) {
               ...
               if (noteTime > audioCtx.currentTime - 0.05) {
                  clickAt(noteTime, n.freq, 0.07, 0.03);
               }
             }
           }
      }
  ...
  let eventsByBeat = []; 
  let measureStartBeats = []; // NEW: Global mixed-meter map

  // Dynamic Playhead State
  ...
  function drawPlayheadAtBeat(beatPos) {
    ...
    // Find which measure contains this beat
    let mIdx = 0;
    // measureStartBeats might be undefined if not generated yet, safe check
    const starts = measureStartBeats || [];
    ...
    // Calculate local beat within that measure
    ...
    exercise.forEach(measure => {
        measure.beats.forEach(beatGroup => {
            ...
            beatGroup.forEach(note => {
               if (note.kind === 'note') {
                   // UNIFIED PITCH: All notes are now 600Hz
                   // We removed the L/R logic so everything sounds consistent
                   const freq = 650; 
  ...
  function scheduleBeats() {
    ...
    while (nextNoteTime < audioCtx.currentTime + LOOKAHEAD) {
      ...
      if (nextBeatIndex < totalBeatsScheduled) {
           // METRONOME: 
           // 1. Check if current beat is the start of a measure (from global map)
           // 2. Use 1200Hz for Downbeat, 900Hz for other beats
           const starts = measureStartBeats || [];
           const isDownbeat = starts.includes(nextBeatIndex);
  ...
  function startMusic(isResuming = false) {
    ...
    audioCtx.resume().then(() => {
      ...
      if (isResuming || accumulatedBeat > 0) {
        ...
        // Count-in: Use length of the FIRST measure (e.g., 4 beats, 3 beats)
        const firstLen = (currentExercise && currentExercise[0]) 
            ? parseInt((currentExercise[0].timeSig || "4/4").split("/")[0], 10) 
            : 4;
  ...
  function regenerate() {
    try {
      ...
      const allowQuarters = !!allowQuartersEl?.checked; // <--- FIXED: Now reading the UI
      ...
      currentExercise = generateExercise({ 
        ...
        restPct, 
        allowQuarters, // <--- FIXED: Now passing to generator
        allow8ths, 
        allow16ths, 
        allowTriplets, 
      ...
      // Build absolute measure start beats for mixed meters
      measureStartBeats = [];
      let acc = 0;
      
      for (const mm of currentExercise) {
          measureStartBeats.push(acc);
          const len = parseInt((mm.timeSig || "4/4").split("/")[0], 10);
          acc += len;
      }
      measureStartBeats.push(acc);
      ...
      if (typeof flattenEvents === "function") {
          ...
          console.warn("flattenEvents() is missing. Playback scheduling will be disabled until it’s added back.");
          eventsByBeat = [];
