...
function makeStaveNote(flow, elem, beatIdx, posInBeat, isTriplet) {
  ...
  function render(exercise) {
    ...
    // Usable width for the staves themselves
    let usableW = wrapW - (marginX * 2);

    // 2. Line Wrapping Strategy (FIX: Prevent Crunching)
    const totalMinW = packs.reduce((sum, p) => sum + p.minW, 0);
    const avgMinW = totalMinW / totalMeasures;
    
    // Constraint A: Density (Add 30% breathing room to the minimum content width)
    const densityLimit = Math.floor(usableW / (avgMinW * 1.3));

    // Constraint B: Fixed Pixel Minimum (Don't allow measures smaller than 280px)
    const pixelLimit = Math.floor(usableW / 280);

    // Combine constraints and HARD CAP at 3 measures per line for spacing
    let maxPerLine = Math.min(densityLimit, pixelLimit, 3); 
    maxPerLine = Math.max(1, maxPerLine); // Ensure at least 1

    // Distribute evenly
    const linesCount = Math.ceil(totalMeasures / maxPerLine);
    const height = marginY * 2 + linesCount * lineGap;

    // 3. Canvas Setup & Scaling
